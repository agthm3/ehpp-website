        ...
        <div class="w-full inset-0 md:relative md:w-1/2">
            <div class="relative hero-image">
              ...
              <div class="absolute w-full h-full">
                ...
                <img src="{{ url('/frontend/images/content/background-image.png') }}" alt="hero 1"
                ...
                @foreach ($pakans as $pakan)
                    <div class="px-4 relative card group">
                        <div class="rounded-xl overflow-hidden card-shadow relative" style="width: 287px; height: 386px">
                            ...
                            <img src="{{ $pakan->galleries()->exists() ? Storage::url($pakan->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}"
                                alt="" class="w-full h-full object-cover object-center" />
                        </div>
                        <h5 class="text-lg font-semibold mt-4">{{ $pakan->name }}</h5>
                        <span class="">Rp {{ number_format($pakan->price) }}</span>
                        <a href="{{ route('details', $pakan->slug) }}" class="stretched-link">
                            <!-- fake children -->
                        </a>
                    </div>
                @endforeach
